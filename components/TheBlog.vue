    ...
    <div class="blog-component mini-spacer pb-0">
      <v-container>
        ...
        >
          <v-card v-if="postDialog">
            ...
            >
              ...
              <v-btn
                icon
                @click="postDialogShow = false"
              >
                <v-icon>mdi-close</v-icon>
              </v-btn>
            </v-toolbar>
            <video
              v-if="postDialog.videoId"
              controls
              width="100%"
            >
              <source
                :src="`${$axios.defaults.baseURL}/videos/${postDialog.videoId}`"
                type="video/mp4"
              >
            ...
            <v-card-text
              class="pt-10"
              v-html="getDescription(postDialog, true)"
            />
          </v-card>
        </v-dialog>
        ...
        <v-row justify="center">
          <v-col
            v-for="(post, i) in posts.slice(0, showMore ? 100 : 3)"
            :key="i"
            cols="12"
            md="6"
            lg="4"
          >
            <v-card
              elevation="0"
              class="blog-card overflow-hidden mb-15"
            >
              <div
                class="position-relative mb-15"
